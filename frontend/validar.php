  ...
  function validarTra() {
    ...
    for (let mt of mtrabajo) {
      ...
      if (inputsFiltrados.length > 0) {
        for (var i = 0; i < valido.length; i++) {
          if (!valido[i] && msgArt[i].id === 'msg-art-' + id[1] + "-" + id[2] && !msgArt[i].querySelector('#tra-' + msgArt[i].id.split('-')[2]) && document.getElementById("trabajos-" + id[1] + "-" + id[2] + "-" + id[3] + "-" + id[4]) && document.getElementById("msg-art-" + id[3]).childElementCount === 1) {
